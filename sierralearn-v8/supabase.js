...
export async function bumpDownload(resourceId) {
  await supabase.rpc('increment_download', { resource_id: resourceId });
}
...
export async function fetchTopResources() {
  const { data: byDownloads } = await supabase.from('resources').select('id,title,downloads,uploader_name,type').eq('status','approved').order('downloads', { ascending:false }).limit(5);
  const { data: byRating }    = await supabase.from('resources').select('id,title,rating_sum,rating_count,uploader_name,type').eq('status','approved').order('rating_count', { ascending:false }).limit(5);
  const { data: byLikes }     = await supabase.from('resources').select('id,title,like_count,uploader_name,type').eq('status','approved').order('like_count', { ascending:false }).limit(5);
